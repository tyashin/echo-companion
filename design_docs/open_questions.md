...
19. Should external context sources — household location, weather, calendar/holidays, news — be ingested as context observations, and with what `origin` marker and confidence semantics? They would serve orientation (§15.2 rule 5), proactive topic selection (§15.5), and interpretation support in evidence bundles (§14), but must remain context for interpretation, never verification of physical claims (§1, §3.1): −20 °C outside does not prove the patient who says «мне холодно» is cold. Note indoor conditions are stronger context than outdoor weather; a smart-home thermometer is now on the roadmap as a Phase 4 data source (question 20).
20. **Smart-home devices (thermometer, humidity, motion/door sensors, …) as a data source** — **SETTLED (as phasing).** Adopted as a later-stage roadmap item (Phase 4, §20): a new device class in the evidence ledger (§7 devices/sessions) emitting timestamped, provenance-bearing observations. They provide the indoor context that question 19 identifies as stronger than outdoor weather (e.g., room temperature when the patient says «мне холодно»). Detailed design — ingestion path (MQTT / Home Assistant bridge vs. direct integration), observation schema, sensor selection — is deferred to Phase 4 planning.
